...
namespace ShwasherSys.ScriptManager
{
    ...
    public class IwbNavigationScriptManager : INavigationScriptManager, ITransientDependency
    {
        public IAbpSession AbpSession { get; set; }

        private readonly IIwbNavigationManager _userNavigationManager;

        public IwbNavigationScriptManager(IIwbNavigationManager userNavigationManager)
        {
        ...
        {
            var userMenus = await _userNavigationManager.GetMenuAsync(AbpSession.ToUserIdentifier());
            ...
            for (int i = 0; i < userMenus.Items.Count; i++)
            {
                AppendMenu(sb, userMenus.Items[i]);
                if (userMenus.Items.Count - 1 > i)
                {
                    sb.Append(" , ");
                }
            }
        ...
        private static void AppendMenu(StringBuilder sb, IwbUserMenuItem menu)
        {
