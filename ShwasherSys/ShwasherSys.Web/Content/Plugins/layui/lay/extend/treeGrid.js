...
layui.define(['laytpl', 'layer', 'form'], function(exports){
    ...
    var $ = layui.$
        ...
        ,TPL_HEADER = function(options){
            ...
            return ['<table cellspacing="0" cellpadding="0" border="0" class="layui-table" '
                , '{{# if(d.data.skin){ }}lay-skin="{{d.data.skin}}"{{# } }} {{# if(d.data.size){ }}lay-size="{{d.data.size}}"{{# } }} lay-even>'//{{# if(d.data.even){ }}lay-even{{# } }}
                ,'<thead>'
    ...
    Class.prototype.exportTree=function(parentId,isExport)
    {
       ...
       // var dbClickI=that.layMain.find('tr[data-id="'+ parentId +'"]').find('.layui-tree-head');
        var dbClickI = that.layMain.find('tr[data-id="' + parentId + '"]').find('.treeHead');
        var dbHtml = dbClickI.html();
        if (dbHtml == undefined || (dbHtml.indexOf("folder") < 0)) {
	        return;
        }
        if(isExport)
        {
            ...
            //dbClickI.html('&#xe625;');
            dbClickI.html("<i class='fa fa-folder-open'></i>");
        }
        else
        {
	        dbClickI.attr('isExport', "false");
            //dbClickI.html('&#xe623;');
	        dbClickI.html("<i class='fa fa-folder'></i>");
                ...
                layui.each(data[parentId], function(i1, item1){
                    ...
                    that.eachCols(function(i3, item3){
                        ...
                        if(field===scopeField)
                        {
                          ...
                          treeHeadImg = temp + "<i class='treeHead' id=treeHead" + item1[paramValue] + " isExport='false'>" + (item1["HasChild"] ? "<i class='fa fa-folder'></i>" : "<i class='fa fa-folder-o'></i>") + " </i>";
                        }
                    ...
                    var tr = '<tr data-index="' + parentId + '_' + i1 + '" data-id="' + item1[paramValue] + '" data-parent-id="' + parentId + '"';
                    var level =Number(item1[levelField]);
					//console.log(level);
	                //if (level>1) {
	                //	if (level % 2 === 0) {
			        //        tr += ' style="background-image: linear-gradient(rgba(250,250,250,0.1),rgba(250,250,250,,0.1))"';
		            //    } else {
			        //        tr += ' style="background-image: linear-gradient(rgba(0,0,0,0.1),rgba(0,0,0,0.1))"';
		            //    }
	                //}
                    //if (item1[levelField] == 2) {
                    //	tr += ' style="background-image: linear-gradient(rgba(250,250,250,0.1),rgba(250,250,250,,0.1))"';
                    //} else if (item1[levelField] == 3) {
                    //	tr += ' style="background-image: linear-gradient(rgba(0,0,0,0.1),rgba(0,0,0,0.1))"';
                    //} else if (item1[levelField] == 4) {
	                //    tr += ' style="background-image: linear-gradient(rgba(0,0,0,0.3),rgba(0,0,0,0.3))"';
                    //}
                    tr +=  '>';
                    ;
    ...
    Class.prototype.events = function(){
       ...
       // that.elem.on('click', 'i.layui-tree-head', function()
        that.elem.on('click', 'i.treeHead', function () {
        	var othis = $(this),
	        othisHtml = othis.html();
        	if (othisHtml.indexOf("folder") > 0&& !othis.find(".fa").hasClass('fa-folder-o') ) {
        ...
        that.elem.on('dblclick', 'td', function() {
	        var othis = $(this), fieldIndex= othis.attr("data-field");
	        if ((/^\+?[1-9][0-9]*$/.test(fieldIndex))) return;
	        if (othis.closest("tr").find("i.treeHead").find("i.fa").hasClass('fa-folder-o')) return;
	        var index = othis.parents('tr').eq(0).data('index')
            ,tr = that.layBody.find('tr[data-index="'+ index +'"]')
            ,request = options.request
            ,response = options.response
